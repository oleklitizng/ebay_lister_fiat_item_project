...
class EBAYHandler:
    ...
    def draft_item_payload(self,
        ...
        Args:
            part_number_str (str): The part number.
            quantity (int): The quantity available.
            condition_id (str): The eBay condition ID (e.g., '1000' for New).
            shipping_profile_id_val (str): The seller's shipping profile ID.
            payment_profile_id_val (str): The seller's payment profile ID.
            return_profile_id_val (str): The seller's return profile ID.
            sku (str): The Stock Keeping Unit for the item.
            item_location (str): Location of the item.
            country_code (str): Country code (e.g., 'DE').
            currency_code (str): Currency code (e.g., 'EUR').
            dispatch_time_max (str): Maximum dispatch time in days.
            vat_percent (float): VAT percentage.
            picture_urls (Optional[List[str]]): List of URLs for item pictures.
            manufacturer_override (Optional[str]): Manually override the manufacturer.
            description_override (Optional[str]): Manually override the description.
            title_override (Optional[str]): Manually override the title.

        Returns:
            dict: The item payload for the eBay API.

        Raises:
            ValueError: If EPERHandler fails to provide essential data or if data is invalid.
        """
        ...
        new_item_payload = {
            'Item': {
                ...
                'SKU': sku,
                # VATDetails benötigt Business Policies für VAT.
                # Wenn nicht verwendet, auskommentieren oder korrekt konfigurieren.
                # 'VATDetails': {'VATPercent': str(vat_percent)}
            }
        }
    ...
    def get_item(self, item_id: str) -> Optional[Dict]:
        ...
        Returns:
            dict: The item data from eBay as a dictionary, or None if an error occurs.
        """
        try:
    ...
    def create_item(self, item_payload: dict) -> Optional[str]:
        ...
        Args:
            item_payload (dict): The item payload, typically from `draft_item_payload`.

        Returns:
            str: The new eBay ItemID if successful, else None.
        """
        try:
    ...
    def revise_item(self, item_id: str, revised_item_fields: dict) -> Optional[str]:
        ...
        Args:
            item_id (str): The eBay ItemID of the listing to revise.
            revised_item_fields (dict): A dictionary containing only the fields to be
                                        revised at the Item level. For example:
                                        {'StartPrice': '29.99', 'Quantity': '5', 'Description': 'New desc'}
                                        Do NOT pass the full item payload from draft_item here.
                                        The ItemID is added automatically.

        Returns:
            str: The ItemID if revision was successful, else None.
        """
